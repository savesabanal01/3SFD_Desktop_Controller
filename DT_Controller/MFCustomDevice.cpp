...
/* **********************************************************************************
    ...
    The custom type is "DT_Controller", which means 14 characters plus NULL = 15
...
void MFCustomDevice::attach(uint16_t adrPin, uint16_t adrType, uint16_t adrConfig, bool configFromFlash)
{
    ...
    ********************************************************************************** */
    ...
    if (strcmp(parameter, "DT_Controller") == 0)
        _customType = DT_CONTROLLER_DEVICE;

    if (_customType ==DT_CONTROLLER_DEVICE) {
        /* **********************************************************************************
            Check if the device fits into the device buffer
        ********************************************************************************** */
        if (!FitInMemory(sizeof(DT_Controller))) {
        ...
        ********************************************************************************** */
        ...
        _myDTController = new (allocateMemory(sizeof(DT_Controller))) DT_Controller(_pin1, _pin2);
        _myDTController->attach(Parameter1, Parameter2);
...
void MFCustomDevice::set(int16_t messageID, char *setPoint)
{
    ...
    if (_customType == DT_CONTROLLER_DEVICE) {
        _myDTController->set(messageID, setPoint);
    }
}
